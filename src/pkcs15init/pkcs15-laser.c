...
laser_init_card_internal(struct sc_profile *profile, struct sc_pkcs15_card *p15card)
{
	...
	if (rv != SC_ERROR_FILE_ALREADY_EXISTS && rv != SC_SUCCESS) {
		...
		LOG_TEST_GOTO_ERR(ctx, rv, "Cannot select Default Admin PIN object.");

	    } else if (!strcmp(to_create[ii], "Aladdin-UserPIN")) {

		rv = laser_create_pin_object(profile, p15card, file, "Default User PIN");
		LOG_TEST_GOTO_ERR(ctx, rv, "Cannot select Default User PIN object.");

	    } else if (!strcmp(to_create[ii], "Aladdin-TransportPIN2")) {

		rv = laser_create_pin_object(profile, p15card, file, "TransportPIN2");
		LOG_TEST_GOTO_ERR(ctx, rv, "Cannot select TransportPIN2 object.");

	    } else if (!strcmp(to_create[ii], "Aladdin-UserPinType")) {

		if (file->size < sizeof(user_pin_type))
		    LOG_TEST_GOTO_ERR(ctx, rv = SC_ERROR_INVALID_DATA, "UserPinType file size is unsufficient");

		if (sizeof(user_pin_type) != sc_pkcs15init_update_file(profile, p15card, file, &user_pin_type, sizeof(user_pin_type)))
		    LOG_TEST_GOTO_ERR(ctx, rv = SC_ERROR_INTERNAL, "Cannot update UserPinType file.");
	    }
	    else if (!strcmp(to_create[ii], "Aladdin-EEED")) {
		unsigned char data[4] = {0x02, 0xD0, 0x01, 0x64};

		if (file->size < sizeof(data))
		    LOG_TEST_GOTO_ERR(ctx, rv = SC_ERROR_INVALID_DATA, "EEED file size is unsufficient");

		if (sizeof(data) != sc_pkcs15init_update_file(profile, p15card, file, data, sizeof(data)))
		    LOG_TEST_GOTO_ERR(ctx, rv = SC_ERROR_INTERNAL, "Cannot update EEEF file");

	    } else if (!strcmp(to_create[ii], "Aladdin-EEEE")) {

		rv = laser_update_eeee(profile, p15card, file);
		LOG_TEST_GOTO_ERR(ctx, rv, "Cannot update EEEE");

	    } else if (!strcmp(to_create[ii], "Aladdin-EEEF")) {

		if (SC_SUCCESS != laser_update_eeef(profile, p15card, file))
		    LOG_TEST_GOTO_ERR(ctx, rv = SC_ERROR_INTERNAL, "Cannot update EEEF");
		...
		LOG_TEST_GOTO_ERR(ctx, rv, "Cannot update CARDID file");
		...
		if (sizeof(data) != sc_pkcs15init_update_file(profile, p15card, file, data, sizeof(data)))
		    LOG_TEST_GOTO_ERR(ctx, rv = SC_ERROR_INTERNAL, "Cannot update CARDCF file");
		...
		if (sizeof(defaults_cardapps) != sc_pkcs15init_update_file(profile, p15card, file, defaults_cardapps, sizeof(defaults_cardapps)))
		    LOG_TEST_GOTO_ERR(ctx, rv = SC_ERROR_INTERNAL, "Cannot update CARDCF file");
	    }
	}
